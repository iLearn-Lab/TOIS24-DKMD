...
def context_know(context, knowledge, context_visual_know):
    ...
    for i in range(batch_size):
        ...
        for each_img_know in each_visual_know:
            if each_img_know != '<pad>' and each_img_know != each_know:  #若文本中涉及多个venue的knowledge，则没法排除重复的
...
def text_train(
    ...
    output_file_train = open('text_244_train.out', 'w',encoding='UTF-8')
    ...
    for epoch_id in range(epoch_id, TextTrainConfig.num_iterations):
        for batch_id, train_data in enumerate(train_data_loader):
            ...
            loss_gener = text_decoder(context, context2, texts1, id2word, output_file_train, image_features, image_len, context_visual_know, text_visual_know)
