    ...
    for i in range(batch_size):
        ...
        if len(each_know_trans) > 0:
            ...
            if len(context_visual_know2.split(' ')) > 70:
                context_visual_know2 = " ".join(context_visual_know2.split(' ')[:70]) 
        else:
...
def text_test(
    ...
    output_file = open('text_244_test.out', 'w', encoding='UTF-8')
    output_file_test_name = 'text_244_test.out'
    ...
    with torch.no_grad():
        for batch_id, test_data in enumerate(test_data_loader):
            ...
            text_decoder.text_generation(context, context2, texts1, id2word, output_file, image_features, image_len, context_visual_know, text_visual_know)
